...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='n-puzzle visulizer')
    ...
    size = int(data[0])
    #Initial Pose
    steps.append([int(x) for x in data[1].split(',')])
    
    gap = steps[0].index(0)
    o_x = int(gap/size); o_y = gap%size
    # data validation check
    for l in data[1:]:
        steps.append([int(x) for x in l.split(',')])
        gap = steps[-1].index(0)
        n_x = int(gap/size); n_y = gap%size
        if (n_x == o_x and abs(n_y-o_y) == 1) or (n_y == o_y and abs(n_x-o_x) == 1):
            o_x=n_x
            o_y=n_y
            continue
        print("ILLEGAL MOVE AT STEP {}".format(len(steps)))
